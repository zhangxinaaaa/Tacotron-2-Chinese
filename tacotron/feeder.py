...
class Feeder:
    ...
    def __init__(self, coordinator, metadata_filename, hparams):
        ...
        with tf.device('/cpu:0'):
            ...
            self._placeholders = [
                ...
                tf.placeholder(tf.float32, shape=(None, None, hparams.num_pit), name='pit_targets'),
            ...
            self.inputs, self.input_lengths, self.mel_targets, self.token_targets, self.pit_targets, self.targets_lengths, self.split_infos = queue.dequeue()

            self.inputs.set_shape(self._placeholders[0].shape)
            self.input_lengths.set_shape(self._placeholders[1].shape)
            self.mel_targets.set_shape(self._placeholders[2].shape)
            self.token_targets.set_shape(self._placeholders[3].shape)
            self.pit_targets.set_shape(self._placeholders[4].shape)
            ...
            self.eval_pit_targets, self.eval_targets_lengths, self.eval_split_infos = eval_queue.dequeue()
            ...
            self.eval_pit_targets.set_shape(self._placeholders[4].shape)
    ...
    def _get_test_groups(self):
        ...
        f32_target = np.fromfile(os.path.join(self._mel_dir, meta[1]), dtype='float32')
    ...
    def _prepare_batch(self, batches, outputs_per_step):
        ...
        for i in range(self._hparams.tacotron_num_gpus):
            ...
            pit_targets_cur_device, pit_target_max_len = self._prepare_targets([x[3] for x in batch],
                                                                                     outputs_per_step)
            pit_targets = np.concatenate((pit_targets, pit_targets_cur_device),
                                            axis=1) if pit_targets is not None else pit_targets_cur_device
            split_infos.append([input_max_len, mel_target_max_len, token_target_max_len, pit_target_max_len])

        split_infos = np.asarray(split_infos, dtype=np.int32)
        return (inputs, input_lengths, mel_targets, token_targets, pit_targets, targets_lengths, split_infos)
