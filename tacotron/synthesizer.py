...
class Synthesizer:
    def load(self, checkpoint_path, hparams, gta=False, model_name='Tacotron'):
        ...
        with tf.variable_scope('Tacotron_model', reuse=tf.AUTO_REUSE) as scope:
            ...
            self.pit_outputs = self.model.tower_pit_outputs if (hparams.predict_pit and not gta) else None
        ...
        if hparams.GL_on_GPU:
            ...
            self.GLGPU_pit_inputs = tf.placeholder(tf.float32, (None, hparams.num_freq), name='GLGPU_pit_inputs')

            self.GLGPU_mel_outputs = audio.inv_mel_spectrogram_tensorflow(self.GLGPU_mel_inputs, hparams)
            self.GLGPU_lin_outputs = audio.inv_linear_spectrogram_tensorflow(self.GLGPU_pit_inputs, hparams)
    ...
    def synthesize(self, texts, basenames, out_dir, log_dir, mel_filenames):
        ...
        if self.gta or not hparams.predict_linear:
            ...
            pitches, mels, alignments, stop_tokens = self.session.run(
                [self.pit_outputs, self.mel_outputs, self.alignments, self.stop_token_prediction], feed_dict=feed_dict)

            # Linearize outputs (1D arrays)
            pitches = [linear for gpu_linear in pitches for linear in gpu_linear]
            mels = [mel for gpu_mels in mels for mel in gpu_mels]
            alignments = [align for gpu_aligns in alignments for align in gpu_aligns]
            stop_tokens = [token for gpu_token in stop_tokens for token in gpu_token]
            ...
            # Take off the batch wise padding
            mels = [mel[:target_length, :] for mel, target_length in zip(mels, target_lengths)]
            pitches = [linear[:target_length, :] for linear, target_length in zip(pitches, target_lengths)]
            # pitches = np.clip(pitches, T2_output_range[0], T2_output_range[1])
            assert len(mels) == len(pitches) == len(texts)
        ...
        for i, mel in enumerate(mels):
            ...
            f32_filename = os.path.join(out_dir, 'f32-{}.npy'.format(basenames[i]))
            f32 = np.concatenate((mel, pitches[i]), axis=-1)
            np.save(f32_filename, f32, allow_pickle=False)
            np.save(mel_filename, mel, allow_pickle=False)
            saved_mels_paths.append(mel_filename)
            saved_f32_paths.append(f32_filename)
            ...
            if log_dir is not None:
                ...
                if hparams.predict_linear:
                    # save wav (linear -> wav)
                    if hparams.GL_on_GPU:
                        wav = self.session.run(self.GLGPU_lin_outputs, feed_dict={self.GLGPU_pit_inputs: pitches[i]})
                        wav = audio.inv_preemphasis(wav, hparams.preemphasis, hparams.preemphasize)
                    else:
                        wav = audio.inv_linear_spectrogram(pitches[i].T, hparams)
                    audio.save_wav(wav, os.path.join(log_dir, 'wavs/wav-{}-linear.wav'.format(basenames[i])),
                                   sr=hparams.sample_rate)

                    # save linear spectrogram plot
                    plot.plot_spectrogram(pitches[i], os.path.join(log_dir, 'plots/linear-{}.png'.format(basenames[i])),
