...
def add_train_stats(model, hparams):
    with tf.variable_scope('stats') as scope:
        ...
        if hparams.predict_pit:
            tf.summary.scalar('pit_loss', model.linear_loss)
            for i in range(hparams.tacotron_num_gpus):
                tf.summary.histogram('pit_outputs %d' % i, model.tower_pit_outputs[i])
                tf.summary.histogram('pit_targets %d' % i, model.tower_pit_targets[i])
...
def add_eval_stats(summary_writer, step, pit_loss, before_loss, after_loss, stop_token_loss, loss):
    values = [
    ...
    if pit_loss is not None:
        values.append(tf.Summary.Value(tag='Tacotron_eval_model/eval_stats/eval_pit_loss', simple_value=pit_loss))
    ...
    with tf.variable_scope('Tacotron_model', reuse=tf.AUTO_REUSE) as scope:
        ...
        if hparams.predict_pit:
            model.initialize(feeder.inputs, feeder.input_lengths, feeder.mel_targets, feeder.token_targets,
                             pit_targets=feeder.pit_targets,
    ...
    with tf.variable_scope('Tacotron_model', reuse=tf.AUTO_REUSE) as scope:
        ...
        if hparams.predict_pit:
            model.initialize(feeder.eval_inputs, feeder.eval_input_lengths, feeder.eval_mel_targets,
                             ...
                             pit_targets=feeder.eval_pit_targets, targets_lengths=feeder.eval_targets_lengths,
                ...
                if step % args.eval_interval == 0:
                    ...
                    if hparams.predict_pit:
                        for i in tqdm(range(feeder.test_steps)):
                            eloss, before_loss, after_loss, stop_token_loss, pit_loss, mel_p, mel_t, t_len, align, lin_p, lin_t = sess.run(
                                [
                                    ...
                                    eval_model.tower_pit_targets[0][0],
                                ])
