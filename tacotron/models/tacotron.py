...
class Tacotron():
    ...
    def initialize(self, inputs, input_lengths, mel_targets=None, stop_token_targets=None, pit_targets=None,
        ...
        with tf.device(split_device):
            ...
            p_pit_targets = tf.py_func(split_func, [pit_targets, split_infos[:, 3]],
                                       lout_float) if pit_targets is not None else pit_targets
            ...
            for i in range(hp.tacotron_num_gpus):
                ...
                if p_pit_targets is not None:
                    tower_pit_targets.append(tf.reshape(p_pit_targets[i], [batch_size, -1, pit_channels]))
        ...
        for i in range(hp.tacotron_num_gpus):
            with tf.device(tf.train.replica_device_setter(ps_tasks=1, ps_device="/cpu:0", worker_device=gpus[i])):
                with tf.variable_scope('inference') as scope:
                    ...
                    pit_condition = hp.predict_pit and not gta
                    ...
                    if pit_condition:
                        self.tower_pit_outputs.append(pit_outputs)
        ...
        self.tower_pit_targets = tower_pit_targets
        ...
        for i in range(hp.tacotron_num_gpus):
            ...
            if pit_condition:
                log('  linear out:               {}'.format(self.tower_pit_outputs[i].shape))
    ...
    def add_loss(self):
                ...
                with tf.variable_scope('loss') as scope:
                    if hp.mask_decoder:
                        ...
                        if hp.predict_pit:
                            ...
                            pit_loss = MaskedLinearLoss(self.tower_pit_targets[i], self.tower_pit_outputs[i],
                        ...
                        if hp.predict_pit:
                            ...
                            l1 = tf.abs(self.tower_pit_targets[i] - self.tower_pit_outputs[i])
                            n_priority_freq = int(2000 / (hp.sample_rate * 0.5) * hp.num_pit)
                            pit_loss = 0.5 * tf.reduce_mean(l1) + 0.5 * tf.reduce_mean(l1[:, :, 0:n_priority_freq])
                        else:
                            pit_loss = 0.
                    ...
                    self.tower_pit_loss.append(pit_loss)

                    tower_loss = before + after + stop_token_loss + regularization + pit_loss
        ...
        self.pit_loss = total_pit_loss / hp.tacotron_num_gpus
